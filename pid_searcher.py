# from: https://github.com/roguelike2d/TekkenBot/blob/master/PIDSearcher.py
...
def getPIDByName(process_name):
    ...
    for index in range(int(BytesReturned.value / ctypes.sizeof(ctypes.wintypes.DWORD))):
        ...
        hProcess = OpenProcess(
            PROCESS_TERMINATE | PROCESS_QUERY_INFORMATION, False, ProcessId
        )
        if hProcess:
            ImageFileName = (ctypes.c_char * MAX_PATH)()
            if GetProcessImageFileName(hProcess, ImageFileName, MAX_PATH) > 0:
                filename = os.path.basename(ImageFileName.value)
                if filename == process_name.encode():
                    pid = ProcessId
